                ...
                <div key={'sp'} className='sprint'>                    
                    <div key={'bkl'} className='backlog'>
                        ...
                        { backlog.length > 0 && backlog.map((incidente,index)=>{
                            return <div className='card' ><p > {incidente.numero_tarea} </p> {incidente.asunto} <p>Dificultad: {incidente.deno_dificultad}</p>  <p>Presupuesto: {Math.round(incidente.tiempo_presupuestado / 60)} Hs.</p> <p>Usado: {Math.round(incidente.total_consumido / 60)} Hs.</p></div>
